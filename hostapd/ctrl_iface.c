...
int send_event_to_agent(struct hostapd_data *hapd,int msg_type,char *msg){
    int sockfd;
    char buf[200];
    sockfd=hapd->agent_sock;
    switch(msg_type)
    {
         case 1:
        	 strcpy(buf,"AP-STA-CONNECTED ");
        	 strcat(buf,hapd->conf->iface);
             strcat(buf," ");
             strcat(buf,msg);
        	 wpa_printf(MSG_ERROR,"***********%s***********",buf);
        	 if(sendto(sockfd,buf,strlen(buf),0,(struct sockaddr *)&hapd->agent_addr,sizeof(struct sockaddr_in))<0){
        	 		wpa_printf(MSG_DEBUG, "Agent: sendto failed: %s",
        	 						   strerror(errno));
        	 		}
        	 break;
         case 2:
        	 strcpy(buf,"AP-STA-DISCONNECTED ");
        	 strcat(buf,hapd->conf->iface);
        	 strcat(buf," ");
        	 strcat(buf,msg);
        	 wpa_printf(MSG_ERROR,"***********%s************",buf);
...
{
	if(hapd->conf->agent_port>0){
		int sockfd;
		if((sockfd=socket(AF_INET,SOCK_DGRAM,0))<0){
				perror("socket:");
				exit(-1);
		 }
		hapd->agent_sock=sockfd;
		struct sockaddr_in agent;
		agent.sin_family=AF_INET;
		agent.sin_port=htons(hapd->conf->agent_port);
		agent.sin_addr.s_addr=inet_addr("127.0.0.1");
		os_memcpy(&hapd->agent_addr,&agent,sizeof(struct sockaddr_in));
    }
#ifdef CONFIG_CTRL_IFACE_UDP
...
void hostapd_ctrl_iface_deinit(struct hostapd_data *hapd)
{
    ...
    if(hapd->agent_sock>-1){
    	close(hapd->agent_sock);
    }
	if (hapd->ctrl_sock > -1) {
#ifndef CONFIG_CTRL_IFACE_UDP
		char *fname;
